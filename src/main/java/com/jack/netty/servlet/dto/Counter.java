...
public class Counter implements Cloneable, Serializable { // NOPMD
	 ...
	 * 
	 * @param name
	 *            Nom du compteur (par exemple: sql...)
	 * @param iconName
	 *            Icône du compteur (par exemple: db.png)
	 */
	 ...
	 *            Nom du compteur (par exemple: sql...)
	 * @param storageName
	 *            Nom unique du compteur pour le stockage (par exemple:
	 *            sql_20080724)
	 * @param iconName
	 *            Icône du compteur (par exemple: db.png)
	 * @param childCounterName
	 *            Nom du compteur fils (par exemple: sql)
	 */
	 ...
	 * 
	 * @param name
	 *            Nom du compteur (par exemple: http...)
	 * @param iconName
	 *            Icône du compteur (par exemple: db.png)
	 * @param childCounter
	 *            Compteur fils (par exemple: sqlCounter)
	 */
	...
	public void addRequestForCurrentContext(String systemErrorStackTrace) {
		...
		if (context != null) {
			final long duration = context.getDuration(System.currentTimeMillis());
			final long cpuUsedMillis = context.getCpuTime();
			addRequest(context.getRequestName(), duration, cpuUsedMillis, systemErrorStackTrace != null, systemErrorStackTrace, -1);
		}
	}
	 ...
	 * 
	 * @param requestName
	 *            Nom de la requête sans agrégation par requestTransformPattern
	 * @return CounterRequest
	 */
	...
	public List<CounterRequestContext> getOrderedRootCurrentContexts() {
		final List<CounterRequestContext> contextList = new ArrayList<CounterRequestContext>(rootCurrentContextsByThreadId.size());
	...
	public Counter clone() { // NOPMD
		// CHECKSTYLE:ON
		final Counter clone = new Counter(getName(), getStorageName(), getIconName(), getChildCounterName(), new ThreadLocal<CounterRequestContext>());
	...
	public String toString() {
		return getClass().getSimpleName() + "[application=" + getApplication() + ", name=" + getName() + ", storageName=" + getStorageName() + ", startDate=" + getStartDate() + ", childCounterName="
			+ getChildCounterName() + ", " + requests.size() + " requests, " + (errors == null ? "" : errors.size() + " errors, ") + "maxRequestsCount=" + getMaxRequestsCount() + ", displayed="
			+ isDisplayed() + ']';
	}
}
