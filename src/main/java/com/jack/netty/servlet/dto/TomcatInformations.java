...
public final class TomcatInformations implements Serializable {
	...
	private TomcatInformations(MBeans mBeans, ObjectName threadPool) throws JMException {
		...
		httpPort = getHttpPort(mBeans, EnvPropertyConfig.getContextProperty(Constant.SYSTEM_SEETING_SERVER_DEFAULT_SERVER_PORT));
	...
	private static String getHttpPort(MBeans server, String defaultPort) {
		...
		try {
			...
			Set<ObjectName> names = server.getAttribute(new ObjectName("Catalina:type=Connector,*"), (QueryExp)null);
