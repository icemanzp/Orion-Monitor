...
public class I18N {
	...
	/**
	 * Retourne la locale pour le thread courant ou la locale par défaut si elle
	 * n'a pas été définie.
	 * 
	 * @return Locale
	 */
	...
	public static ResourceBundle getResourceBundle() {
		...
		if (Locale.ENGLISH.getLanguage().equals(currentLocale.getLanguage())) {
			// there is no translations_en.properties because
			// translations.properties is in English
			// but if user is English, do not let getBundle fallback on server's
			// default locale
			return ResourceBundle.getBundle(RESOURCE_BUNDLE_BASE_NAME, ROOT_LOCALE);
		}
		// and if user is not English, use the bundle if it exists for his/her
		// Locale
	 ...
	 * 
	 * @param key
	 *            clé d'un libellé dans les fichiers de traduction
	 * @return String
	 */
	...
	/**
	 * Retourne une traduction dans la locale courante et l'encode pour
	...
	public static String getStringForJavascript(String key) {
		...
		// ici, le résultat ne contient pas de valeur variable ni d'attaque
		// puisque ce sont des messages internes et fixes,
		// donc pas besoin d'encoder avec javascriptEncode, et on conserve les
		// apostrophes lisibles dans les messages
		return string.replace("\\", "\\\\").replace("\n", "\\n").replace("\"", "\\\"").replace("'", "\\'");
	}

	/**
	 * Retourne une traduction dans la locale courante et insère les arguments
	 * aux positions {i}.
	 * 
	 * @param key
	 *            clé d'un libellé dans les fichiers de traduction
	 * @param arguments
	 *            Valeur à inclure dans le résultat
	 * @return String
	 */
	public static String getFormattedString(String key, Object... arguments) {
		// échappement des quotes qui sont des caractères spéciaux pour
		// MessageFormat
	...
	/**
	 * Écrit un texte dans un flux en remplaçant dans le texte les clés
	 * entourées de deux '#' par leurs traductions dans la locale courante.
	 * 
	 * @param html
	 *            texte html avec éventuellement des #clé#
	 * @param writer
	 *            flux
	 * @throws IOException
	 *             e
	...
	/**
	 * Écrit un texte, puis un retour chariot, dans un flux en remplaçant dans
	 * le texte les clés entourées de deux '#' par leurs traductions dans la
	 * locale courante.
	 * 
	 * @param html
	 *            texte html avec éventuellement des #clé#
	...
	public static DecimalFormat createIntegerFormat() {
		// attention ces instances de DecimalFormat ne doivent pas être
		// statiques
	...
	public static DateFormat createDateFormat() {
		...
		// voir
		// http://java.sun.com/javase/6/docs/api/java/text/DateFormat.html#synchronization
